...
class DWTForward(nn.Module):
    ...
    def __init__(self, J=1, wave='db1', mode='zero'):
        ...
        self.filts0 = nn.Parameter(filts[0], requires_grad=False)
        self.filts1 = nn.Parameter(filts[1], requires_grad=False)
        self.filts2 = nn.Parameter(filts[2], requires_grad=False)
        self.filts3 = nn.Parameter(filts[3], requires_grad=False)
        self.register_buffer('h0_col', self.filts0)
        self.register_buffer('h1_col', self.filts1)
        self.register_buffer('h0_row', self.filts2)
        self.register_buffer('h1_row', self.filts3)
        self.J = J
        self.mode = mode
...
class Level1Waveblock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.dwt = DWTForward(J=1, mode='zero',
                              wave='db1')

        self.feedforward = nn.Sequential(
            nn.Conv2d(final_dim * 2, final_dim * mult, 1),
            nn.GELU(),
            nn.Dropout(dropout),
        ...
        self.encode_layer = HybridEncoder( hidden_dim=ff_channel ,point_cloud_range=[-54.0, -54.0, -5.0, 54.0, 54.0, 3.0])
        self.reduction = nn.Conv2d(final_dim, int(final_dim / 4), 1)

    def forward(self, x):
        b, c, h, w = x.shape

        x1 = self.encode_layer(x)
        ...
        # x1 = self.p_out_porj(torch.cat((x, x1), dim=1))
...
class WaveMixModule(nn.Module):
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(
                Level1Waveblock(mult=mult, ff_channel=ff_channel, final_dim=final_dim, dropout=dropout, input_img=input_img))
        ...
        self.decoder1 = nn.Sequential(
            nn.Conv2d(final_dim * 2, int(final_dim), 3, padding=1, ),
            nn.BatchNorm2d(int(final_dim))
        )
    ...
    def __init__(
        ...
        for _ in range(num_modules):
            self.wavemodules.append(
                WaveMixModule(depth=depth, mult=mult, ff_channel=ff_channel, final_dim=final_dim,
                              dropout=dropout,input_img=input_img))
...
if __name__ == '__main__':
    b, c, h, w = 2, 128, 180, 180
    timsteps = 100
    model = WavePaint().cuda()
